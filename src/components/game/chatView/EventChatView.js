...
const EventChatView = ({navigation, route}) => {
    ...
    const handleSend = () => {
        ...
        setInputMessage("");
        const ref = firebase.firestore().collection("eventChat").doc();
    ...
    useEffect(() => {
        ...
        if (eventID !== null) {
            unsubscribe = firebase.firestore().collection("eventChat").where("eventID", "==", eventID).orderBy("when", "desc")
