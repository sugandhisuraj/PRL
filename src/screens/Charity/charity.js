...
const Charity = ({ navigation }) => {
  ...
  const loadCharityData = async () => {
    ...
    let removePlayerD = charityData.filter((i) => i.charityID != 0 && i.charityType === "Charity");
    let sortedCharityData = sortArrayAlphabatically(
      removePlayerD,
      "charityName"
    );
              ...
              >
                ...
                <View>
                  <ImageVideoPlaceholder
                    ...
                    renderChildren
                    disabledOnPress={
                      charityArray[`${userChoice}`]?.charityVideo?.length == 0
                    }
                  >
                    {charityArray.length > 0 &&
                    charityArray[`${userChoice}`]?.charityVideo?.length == 0 ? (
